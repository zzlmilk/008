...
class PHPMailer {
     ...
     * 
     */
    public function sendData($email_title,$reception_email,$body) {
        $this->IsSMTP(); // 启用SMTP
        $this->CharSet = "UTF-8"; //字符集
        $this->Encoding = "base64"; //编码方式
        $this->Subject = $email_title; //邮件标题
        $this->Body = $body; 
        $this->stmpConfig();
        $this->AddAddress($reception_email,$reception_email); //添加收件人（地址，昵称）
     ...
     * 
     */
    public function stmpConfig(){
        $this->Host = 'smtp.qq.com';
        $this->Username = '372260652@qq.com';
        $this->Password = 'lw83471961006';
        
         $this->From = '372260652@qq.com';  //发件人地址（也就是你的邮箱）
        $this->FromName = '372260652@qq.com';  //发件人姓名
    }
    /**
     * 
     */
    public function cc_email($cc){
        if(!empty($cc)){
            $cc_list = explode(',', $cc);
            if(count($cc_list) > 0  && is_array($cc_list)){
